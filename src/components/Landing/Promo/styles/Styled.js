...
export const PromoStyled = styled.article`
  ...
  background: ${props => props.background &&
      `url(${props.background}${!!props.responsive ? "" : "-xs"}.${props.fileExtension || "jpg"}) 0 100% / ${props.backgroundSize || "cover"} no-repeat transparent;`};
  background-position: ${props => props.backgroundSize && "center"};
  display: flex;
  justify-content: ${props =>
    props.direction === "end" ? "flex-end" : props.direction === "start" ? "flex-start" : "center"};
  align-items: center;
  img {
    max-width: 100%;
  }
  ...
  ${media.large`
    ...
    background-position: ${props =>
      props.backgroundSize ? "center" : props.direction === "end" ? "60% 100%" : "20% 100%"};
